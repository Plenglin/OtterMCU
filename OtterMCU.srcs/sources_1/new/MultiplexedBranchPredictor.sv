...
module MultiplexedBranchPredictor(
    ...
    BranchPredictor always_ibp(.*, .should_branch(always_should_branch));
    AlwaysBranchPredictor always_bp(.bp(always_ibp));
    
    logic never_should_branch;
    BranchPredictor never_ibp(.*, .should_branch(never_should_branch));
    NeverBranchPredictor never_bp(.bp(never_ibp));
    
    logic backwards_should_branch;
    BranchPredictor backwards_ibp(.*, .should_branch(backwards_should_branch));
    AlwaysBackwardsPredictor backwards(.bp(backwards_ibp));
    
    logic random_should_branch;
    BranchPredictor random_ibp(.*, .should_branch(random_should_branch));
    RandomBranchPredictor random(.bp(random_ibp));
    
    logic tla_should_branch;
    BranchPredictor tla_ibp(.*, .should_branch(tla_should_branch));
    TwoLevelAdaptivePredictor tla(.bp(tla_ibp));
    
    logic tbs_should_branch;
    BranchPredictor tbs_ibp(.*, .should_branch(tbs_should_branch));
    ...
    always_comb case (selection)
        ...
        bp_backwards: 
            bp.should_branch = backwards_should_branch;
        bp_tla: 
